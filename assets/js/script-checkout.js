// ✅ Load products (same as in cart.js)
const products = [
...
function renderOrderSummary() {
  ...
  cart.forEach(item => {
    ...
    if (product) {
      ...
      html += `<p>${product.name} (x${item.quantity}) <span>₹${itemTotal}</span></p>`;
    }
  });

  html += `<hr><p class="total">Total <span>₹${totalAmount}</span></p>`;
...
// ✅ Initial render
renderOrderSummary();
// Toggle payment method input fields
